...
``golden_sfap`` is the **validated Fourier pipeline** (`muap_generator.fourier`,
...
Run: python \
        muap_generator/spatial_refactor/verification/build_golden.py
"""
...
from muap_generator.preprocessing import resample_centered_line
...
def fourier_golden(phi, dz, L1, L2, v, posz=0.0):
    ...
    Includes the posz phase term (NMJ offset) exactly as muap_generator.fourier."""
    zs = v * 1000.0 / FS
...
def main():
    ...
    out["reference"] = np.array("muap_generator.fourier (validated r=0.997 vs MATLAB)")
