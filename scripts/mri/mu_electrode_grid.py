...
def main():
    ...
    bed = build_muscle_beds(fm, density=4.0, method="poisson", labels=[FCU], min_fibers=30)[FCU]
    ...
    # ---- solve the grid ONCE, sample φ along every bed fibre ----
    print(f"solving {M*M} electrodes over FCU (θ={fcu_ang:.0f}°±{args.dtheta:.0f}, "
    ...
    for mu_idx in mus_req:
        ...
        _plot(mu_idx, mu.size, M, t_ms, W, p2p, ied_z, ied_t)
    print(f"done in {time.time()-t0:.0f}s → {OUT}")


def _plot(mu_idx, mu_size, M, t_ms, W, p2p, ied_z, ied_t):
    ...
    fig.suptitle(f"HD-EMG grid ({M}×{M}, IED ≈ {ied_z:.0f}×{ied_t:.0f} mm) — MU{mu_idx} "
                 f"({mu_size} fibres), monopolar MUAP per electrode · shared ±{ymax:.1f}µV",
                 fontsize=12)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(OUT / f"grid_mu{mu_idx}_{M}x{M}.png", dpi=130, bbox_inches="tight")
    plt.close(fig)
