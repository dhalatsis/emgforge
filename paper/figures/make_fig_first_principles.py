...
(b) the Fourier engine vs the same oracle, (c) engine/oracle amplitude ratio vs CV (= 1/v),
(d) the monopole-free source identity.
...
NUM["c_ratio_times_v"] = {f"v{v_:g}": ratio[v_] * v_ for v_ in vs}
...
# figure --------------------------------------------------------------------
fig = plt.figure(figsize=(W2, 2.05))
gs = fig.add_gridspec(1, 4, width_ratios=[1.35, 1.2, 0.95, 0.85], wspace=0.5, left=0.06, right=0.99, top=0.9, bottom=0.21)

ax = fig.add_subplot(gs[0, 0])
for k, p in enumerate(pairs):
    off = -1.6 * k
    ax.plot(p["t_r"], normed(p["s_r"]) + off, color=COL["first"], lw=1.8, alpha=0.35,
            label="closed-form oracle" if k == 0 else None, solid_capstyle="round")
    ax.plot(p["t_s"], normed(p["s_s"]) + off, color=COL["golden"], lw=0.8, ls="--",
            label="spatial engine" if k == 0 else None)
    lag = 0.0 if abs(p["lag"]) < 0.005 else p["lag"]
    ax.text(29.5, off + 0.12, f"NMJ {p['posz']:+g} mm\nr = {p['r']:.4f}, lag {lag:.2f} ms",
            fontsize=6, ha="right", va="bottom")
ax.set_xlim(-2, 30); ax.set_yticks([]); ax.set_ylim(-4.4, 1.35)
ax.set_xlabel("t (ms), t = 0 at the NMJ"); ax.set_ylabel("SFAP (norm., offset)")
ax.legend(loc="lower left", handlelength=1.5, borderaxespad=0.0, fontsize=6.2, bbox_to_anchor=(0.1, 1.0), ncol=2,
...
ax.plot(t_r, normed(s_r), color=COL["first"], lw=1.8, alpha=0.35, label="closed-form oracle")
ax.plot(t_f, normed(s_f), color=COL["fourier"], lw=0.9, label="Fourier engine")
ax.axhline(0, color="0.85", lw=0.5, zorder=0)
ax.set_xlim(-2, 30); ax.set_ylim(-1.2, 1.45)
ax.set_xlabel("t (ms), t = 0 at the NMJ"); ax.set_ylabel("SFAP (norm.)")
ax.legend(loc="upper right", handlelength=1.5, borderaxespad=0.1, fontsize=6.2)
ax.text(0.03, 0.04, f"NMJ −20 mm\nsigned r = {r_f:+.2f} at lag {lag_f:+.1f} ms", transform=ax.transAxes,
        fontsize=6, ha="left", va="bottom")
letter(ax, "b", dx=-0.2)

ax = fig.add_subplot(gs[0, 2])
vv = np.linspace(1.6, 5.6, 100)
ax.plot(vv, 1 / vv, color="0.6", lw=0.8, ls="--", label="1 / v")
ax.plot(vs, [ratio[v_] for v_ in vs], "o", color=COL["golden"], ms=4, mec="white", mew=0.5, label="engine / oracle")
for v_ in vs:
    ax.text(v_ + 0.13, ratio[v_] + 0.012, f"{ratio[v_]:.3f}", fontsize=5.5, ha="left", va="bottom")
ax.set_xlim(1.6, 5.6); ax.set_ylim(0, 0.62); ax.set_xticks(vs)
ax.set_xlabel("conduction velocity v (m/s)"); ax.set_ylabel("amplitude ratio")
ax.legend(loc="lower left", handlelength=1.5, borderaxespad=0.3, fontsize=6.2)
letter(ax, "c", dx=-0.3)
...
ax.set_yscale("log"); ax.set_ylim(1e-18, 3)
ax.axhline(1e-6, color="0.5", lw=0.5, ls=(0, (2, 1.5))); ax.text(2.4, 1.6e-6, "gate 10$^{-6}$", fontsize=5.5, ha="right", va="bottom", color="0.4")
...
for x_, v_ in zip([0, 1, 2], vals):
    ax.text(x_, v_ * 3, f"{v_:.0e}".replace("e-", "e−"), fontsize=5.5, ha="center", va="bottom")
letter(ax, "d", dx=-0.42)
