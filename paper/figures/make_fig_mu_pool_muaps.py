...
gsb = gs[0, 1].subgridspec(1, 2, wspace=0.45)
...
axr.scatter(sizes, np.arange(len(pool)), s=7, c=sizes, cmap="viridis", norm=snorm, lw=0)
for k in pk:
    axr.annotate(pick_lab[k], xy=(sizes[k], k), xytext=(-7, 2), textcoords="offset points", fontsize=7, fontweight="bold")
axr.set_xscale("log"); axr.set_xlabel("fibres per MU"); axr.set_ylabel("recruitment order")
axr2 = axr.twinx()
axr2.plot(sizes, rte, color="0.3", lw=0.8)
axr2.set_ylabel("recruitment threshold\n(fraction of max drive)", fontsize=6.5, color="0.3")
axr2.tick_params(axis="y", labelsize=6, colors="0.3"); axr2.spines["right"].set_visible(True)
letter(axh, "b", dx=-0.4)
...
cb.set_label("MU centre → electrode (mm)", fontsize=7); cb.ax.tick_params(labelsize=6)
letter(ax, "d")
