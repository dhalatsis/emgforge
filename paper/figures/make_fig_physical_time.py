...
for ax, (stack, let, col, tag) in zip(axes, panels):
    ...
    # propagating guide: t = |z| / v (wave-front arrival)
    ax.plot(np.abs(ZC) / V, np.arange(len(ZC)) * STEP, color="0.35", lw=0.7, ls=(0, (3, 2)), zorder=1)
    ...
    ax.set_xlim(-2, 24); ax.set_ylim(-0.8, len(ZC) * STEP - 0.2)
...
axes[0].text(L / V + 0.3, -0.7, "L/v", fontsize=6, ha="left", va="bottom", color="0.3")
axes[0].text(np.abs(ZC[0]) / V + 0.5, 0.35, "|z|/v", fontsize=6, ha="left", va="bottom", color="0.3", rotation=0)
axes[0].annotate("EOF", (L / V + 1.2, 6.55), (19.5, 7.3), fontsize=6, ha="left", va="bottom", color="0.3",
...
axes[1].annotate("IZ: null,\nphase reversal", (2.5, 4.0), (9.0, 3.3), fontsize=6, ha="left", va="center", color="0.3",
                 arrowprops=dict(arrowstyle="->", lw=0.4, color="0.4"))
axes[2].annotate("EOF softened", (L / V - 1.5, 6.55), (17.5, 7.3), fontsize=6, ha="left", va="bottom", color="0.3",
                 arrowprops=dict(arrowstyle="->", lw=0.4, color="0.4"))
