...
A technical report describing the whole chain, its validation and the released datasets
is in [`paper/`](paper/) (arXiv preprint in preparation).
...
paper/               the technical report (LaTeX, figure scripts, dataset manifests)
...
| tier | scope | status |
|---|---|---|
| A | cylinder: first principles → analytical → FEM → pipeline | 14/15 (4 known) |
| B | MUAP features vs the literature | 14/14 |
| C | interference EMG & motor-unit pool (released D2 bank) | 4/5 (1 known) |
| S | chain-level sanity (released D2 bank) | 7/7 (1 known) |

Headlines: the spatial engine reproduces a closed-form line-source oracle at r = 1.0000 with
zero lag and a monopole-free source; end-of-fibre onset at L/v within 0.4 ms; CV, innervation-
zone, end-of-fibre, depth, fat, electrode-size and IED laws match the literature. Known open
items (kept with their real criteria so they turn green when fixed): the engine's amplitude
constant is off by exactly 1/v; the Fourier engine is anti-phase and lagged vs first
principles; on FEM lead fields the golden amplitude is erratic at ±40 % (shape and spectrum
are faithful); forearm MUAPs are of physiological duration (18 ms) but several times too
small (median 5.9 µV) — the amplitude constant plus the straight-fibre geometry (C-04).
Tiers C/S take `EMGFORGE_MUAP_BANK=<forearm_fcu_mu_pool.npz>` to run on the released pool.
...
Halatsis, D. (2026). *emgforge: an open, validated forward model of surface EMG — from
volume-conductor lead fields to motor-unit action potentials and interference signals.*
Technical report, `paper/main.pdf` (arXiv submission in preparation).
